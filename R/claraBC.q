...
claraBC <- function(x, k, metric = "euclidean", stand = FALSE,
                  ...
                  pamLike = FALSE)
{
    ...
    doDUP <- nzchar(dup <- Sys.getenv("R_vmsbase_claraBC_dup")) && isTRUE(as.logical(dup))
    if((trace <- as.integer(trace)))
	cat(sprintf("calling .C(cl_claraBC, ..., DUP = %s):\n", doDUP))
	
	cat((if(metric == "manhattan") "metric 2 - Manhattan\n" else(if(metric == "euclidean") "metric 1 - Euclidean\n" else "metric 3 - Bray-Curtis\n")), sep = "")
    ...
    res <- .C("cl_claraBC",
	      ...
	      ndyst = as.integer(if(metric == "manhattan") 2 else(if(metric == "euclidean") 1 else 3) ),
              as.logical(rngR[1]),
	...
	## else {cannot happen}
	stop("invalid 'jstop' from .C(cl_claraBC,.): ", res$jstop)
    }
    ...
    class(r) <- c("claraBC", "partition")
    r
}

print.claraBC <- function(x, ...)
{
...
summary.claraBC <- function(object, ...)
{
    class(object) <- "summary.claraBC"
    object
}

print.summary.claraBC <- function(x, ...)
{
    cat("Object of class 'claraBC' from call:\n", deparse(x$call),
	"\nMedoids:\n");		print(x$medoids, ...)
    cat("Objective function:\t ", format(x$objective, ...),
