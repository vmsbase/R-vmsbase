...
gui_mark_fis_poi <- function(vms_db_name = "", harb_file_name = "") {
  ...
  mark_fis_poi_win <- gwindow(
    title = "Mark Fishing Points Tool",
    visible = FALSE,
    width = 950,
    height = 350
  )
  ...
  gimage(system.file("ico/folder-man.png", package = "vmsbase"),
    container = cus_har_g,
    handler = function(h, ...) {
      harb$path <- gfile(
        text = "Select ShapePoints map",
        type = "open",
  ...
  met_da_su <- gdroplist(c("VMS-LB Match", "NN Prediction"),
    selected = 1, container = dat_sel_f,
    handler = function(h, ...) {
