...
gui_main <- function() {
  ...
  gimage(system.file("ico/folder-html.png", package = "vmsbase"),
    ...
    handler = function(h, ...) {
      themap_file <<- gfile(
        text = "Select Land ShapePoly map",
        type = "open",
  ...
  save_ws_b <- gbutton(text = "Save\nWorkSpace", container = sa_lo_ws_g, handler = function(h, ...) {
    ...
    if (!is.na(ws_file)) {
      ...
      gmessage(paste("WorkSpace not saved!\n\nPlease enter a valid name and\ndestination of the WorkSpace file", sep = ""),
        title = "WorkSpace Save Error!",
        icon = "error"
      )
    }
  })
  ...
  load_ws_b <- gbutton(text = "Load\nWorkSpace", container = sa_lo_ws_g, handler = function(h, ...) {
    ...
    if (!is.na(ws_file)) {
      ...
      gmessage(paste("WorkSpace not loaded!\n\nPlease select a valid WorkSpace file", sep = ""),
