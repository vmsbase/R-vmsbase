...
gui_lb_editraw <- function() {
  ...
  addSpring(g1a)
  loadbutt <- gbutton("Load Raw LogBook data",
    container = g1a,
    handler = function(h, ...) {
  ...
  g3_met_r <- gradio(c("Yes", "No"),
    horizontal = T, container = g3_meti,
    handler = function(h, ...) {
  ...
  g3_gea_r <- gradio(c("Yes", "No"),
    horizontal = T, container = g3_gear,
    handler = function(h, ...) {
  ...
  gbutton("ok", container = g8, handler = function(h, ...) {
    ...
    if (svalue(saveModeSel) == "Append") {
      ...
      file = gfile(
        text = "Append Edited LogBook Data to existing file:",
        type = "save",
        filter = list("logBook files" = list(patterns = c("*.logbook")))
      ),
      sep = ",",
