...
gui_lb_met_cla <- function(lb_db_name = "") {
  ...
  lb_met_cla_win <- gwindow(
    title = "LogBook Metier Classification Tool",
    visible = FALSE
  )
  ...
  in_by_sel <- gradio(c(
    "Stored 'Metier' field in DB",
    "Stored 'Gear' field in DB",
    "Fuzzy Membership of Catch Profile"
  ),
  container = in_sel_f, horizontal = FALSE,
  handler = function(h, ...) {
  ...
  gimage(system.file("ico/document-properties.png", package = "vmsbase"),
    container = img_g,
    handler = function(h, ...) {
      if (lb_DB$db != "") {
  ...
  sta_cla_sel <- gradio(c("Yes", "No"),
    container = me_cl_st_g, horizontal = FALSE,
    handler = function(h, ...) {
