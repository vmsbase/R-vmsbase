...
void cl_claraBC(int *n,  /* = number of objects */
	      ...
	      int *diss_kind,/*= {1,2};  1 : euclidean;  2 : manhattan;  3 : bray-curtis*/
...
#define ntmp6 &itmp[2*nsamb+ *nsam]
    ...
    *jstop = 0;
    ...
    if(*trace_lev) Rprintf("C claraBC(): (nsam,nran,n) = (%d,%d,%d);%s\n",
			   *nsam, *nran, *n,
...
/* __LOOP__ :  random subsamples are drawn and partitioned into kk clusters */
	...
	if(*trace_lev) Rprintf("C claraBC(): sample %d ", jran);
	...
	for(l = 1; l < n_dys; l++) /* dys[0] is not used here */
		...
		s = dys[l];
	if(*trace_lev >= 2)
	    Rprintf(". claraBC(): s:= max dys[1..%d] = %g;", l-1,s);
...
/* --- end random sampling loop */
    ...
    if(*trace_lev) {
	Rprintf("C claraBC(): best sample _found_ ");
	if(*trace_lev >= 2) {
	    Rprintf("; nbest[1:%d] =\n c(", *nsam);
	    for (js = 0; js < *nsam; ++js) {
		Rprintf("%d", nbest[js]);
...
/*     for the best subsample, the objects of the entire data set
    ...
    *obj = zba / rnn;
    ...
    if(dyst_toomany_NA) {
	REprintf(" *** SHOULD NOT HAPPEN: claraBC() -> dysta2(nbest) gave toomany_NA\n");
	return;
    }
...
/* Compute Dissimilarities for the selected sub-sample	---> dys[,] */
	...
	for (int k = 0; k < l; ++k) { /* compute d(nsel[l], nsel[k]) {if possible}*/
		...
		*toomany_NA = TRUE;
		...
		dys[nlk] = (diss_kind == 3) ? (1 - (d1/dlk)) : d1 ;
	    }
	} /* for( k ) */
    } /* for( l ) */
    return;
	   ...
	   int *jtmd, double *valmd, double *x, int *nrx, int *mtt)
{
    ...
    double pp = (double) (jpp), dsum, dlk, tra, dnull = -9./* Wall */;
	    ...
	    for (jk = 0; jk < kk; ++jk) {
			...
			for (j = 0; j < jpp; ++j) {
				...
				if (diss_kind == 1 || diss_kind == 2) {
					...
					tra = 2 * fmin(x[nrjk + j * n], x[njnb + j * n]);
					dlk += x[nrjk + j * n] + x[njnb + j * n];
				}
